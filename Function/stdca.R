stdca <- function(data, outcome, ttoutcome, timepoint, predictors, xstart=0.01, xstop=0.99, xby=0.01, 
  ...
  if (graph==TRUE) {
    ...
    if(intervention==TRUE) {
      ...
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(interv$threshold,data.matrix(interv[paste(predictors[m],"_sm",sep="")]),col=m,lty=1)     
        } else {
          lines(interv$threshold,data.matrix(interv[predictors[m]]),col=m,lty=1)
        }
    ...
    # PLOTTING NET BENEFIT IF REQUESTED
      ...
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(nb$threshold,data.matrix(nb[paste(predictors[m],"_sm",sep="")]),col=m,lty=1) 
        } else {
          lines(nb$threshold,data.matrix(nb[predictors[m]]),col=m,lty=1)
        }
        # adding each model to the legend
        legendlabel <- c(legendlabel, predictors[m])
        legendcolor <- c(legendcolor, m)
